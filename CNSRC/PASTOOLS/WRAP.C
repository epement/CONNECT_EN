...
#define Qsize  sizeof (Qpr) / sizeof ( struct	Qpref )
...
char *wordc;
{
	...
	{
		lpref =	strlen(	s->pref	);	// [BT]
		if(lpref > lword) continue;
		if(strncmp(s->pref, wordc, lpref) != 0)	continue;
		cp = s->locks;
		while(*cp)
			if(strncmp(&wordc[lpref], cp++,	2) == 0) goto Cont;
			else cp++;
