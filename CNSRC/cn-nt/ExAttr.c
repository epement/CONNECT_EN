...
BOOL SetComprAttr(char *fName, DWORD Compress)
{
   ...
   if (hDevice != INVALID_HANDLE_VALUE)
     {
      Res = DeviceIoControl(
              hDevice,            // handle to file or directory
              FSCTL_SET_COMPRESSION,       // dwIoControlCode
              &Compress,           // input buffer
              sizeof(Compress),    // size of input buffer
              NULL,                        // lpOutBuffer
              0,                           // nOutBufferSize
...
BOOL SetFAttr(char *fName, DWORD fAttr)
{
 ...
 if (((Attrs & FILE_ATTRIBUTE_COMPRESSED) != 0) &&
     ((fAttr & FILE_ATTRIBUTE_COMPRESSED) == 0))
  Res = SetComprAttr(fName, COMPRESSION_FORMAT_NONE);

 /* Set compressed */
 if (((Attrs & FILE_ATTRIBUTE_COMPRESSED) == 0) &&
     ((fAttr & FILE_ATTRIBUTE_COMPRESSED) != 0))
  {
   Res = SetComprAttr(fName, COMPRESSION_FORMAT_DEFAULT);
   fAttr &= ~FILE_ATTRIBUTE_COMPRESSED;
  }

 /* Clear encrypted */
 if (((Attrs & FILE_ATTRIBUTE_ENCRYPTED) != 0) &&
     ((fAttr & FILE_ATTRIBUTE_ENCRYPTED) == 0))
  Res = DecryptFile(fName, 0);

 /* Set encrypted */
 if (((Attrs & FILE_ATTRIBUTE_ENCRYPTED) == 0) &&
     ((fAttr & FILE_ATTRIBUTE_ENCRYPTED) != 0))
  {
   Res = EncryptFile(fName);
   fAttr &= ~FILE_ATTRIBUTE_ENCRYPTED;
  }
