...
void docyrix(int maxi){
  ...
  if(maxi >= 1){
    ...
    if(family == 5 && model == 0){
      ...
      for(i=0;i<32;i++){
        if(edx & (1<<i)){
          printf("%s\n\r",Cyrix_standard_feature_flags_5[i]);
        }
      }
    } else {
      int i;
      ...
      for(i=0;i<32;i++){
        if(edx & (1<<i)){
          printf("%s\n\r",Cyrix_standard_feature_flags_not5[i]);
        }
      }
    }
  }
  ...
  if(maxei >= 0x80000001){
    ...
    for(i=0;i<32;i++){
      if(edx & (1<<i)){
        printf("%s\n\r",Cyrix_extended_feature_flags[i]);
      }
    }
  }
  printf("\n\r");
