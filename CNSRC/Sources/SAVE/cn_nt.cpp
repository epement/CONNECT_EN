...
/*        memmove( gptr, srcptr, size ); */
   ...
   uBufSize = MultiByteToWideChar( CP_OEMCP, 0, srcptr,
        size+1, buf, 0);

   usize = MultiByteToWideChar( CP_OEMCP, 0, srcptr,
        size+1, buf, uBufSize);

        gmem = GlobalAlloc(GMEM_MOVEABLE|GMEM_DDESHARE, uBufSize );
        if(!gmem) { setAX(GetLastError()); return; };
