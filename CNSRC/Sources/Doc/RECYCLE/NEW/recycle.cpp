...
//
// RECYCLE.cpp -- command line utility to move a file to the Recycle Bin.
...
BOOL		ConfirmEmpty(LPCTSTR pszRoot);
BOOL		ConfirmDelete(LPCTSTR pFileName);
char		GetEchoChar();
...
BOOL bEmptyBin=FALSE;		// empty the recycle bin
...
int main(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	if (bEmptyBin) {
		// empty the recycle bin. Arg is which drive/path to empty, or none for
		// all drives.
		LPCTSTR pszRoot = files.empty() ? NULL : files.front().c_str();
		if (fileexists(pszRoot)) {
			if (bQuiet || ConfirmEmpty(pszRoot)) {
				HRESULT hr = SHEmptyRecycleBin(NULL, pszRoot, SHERB_NOCONFIRMATION);
				if (!SUCCEEDED(hr)) {
					TCHAR buf[512];
					IErrorInfo* perrinfo;
					if (GetErrorInfo(0,&perrinfo)==S_OK) {
						...
						_tprintf("Error 0x%8x\n", hr);
					}
				}
			} else {
				_tprintf("Not emptied\n");
			}
		} else {
			_tprintf("Folder doesn't exist: %s\n", pszRoot);
		}
		...
		for (it=files.begin(); it!=files.end(); it++) {
			...
			if (fileexists(pFileName)) {
				...
				if (!bDisplayOnly) {
					if (!bPrompt || ConfirmDelete(pFileName)) {
...
void usage()
{
	_tprintf(_T("Usage: RECYCLE [/QNPZE?] file...\n"));
}
...
void help()
{
   ...
   _tprintf(_T(" /E(mpty)     empty bin--arg is drive letter or none for all\n"));
}
...
BOOL ConfirmDelete(LPCTSTR pFileName)
{
	char pmt[2*_MAX_PATH];
	_stprintf(pmt, _T("Recycle %s (Y/N/All)? "), pFileName);
	while (TRUE) {
		_tprintf(pmt);
		char c = GetEchoChar();
		switch (tolower(c)) {
...
BOOL ConfirmEmpty(LPCTSTR pszRoot)
{
	if (!pszRoot)
		pszRoot="";
	char pmt[2*_MAX_PATH];
	_stprintf(pmt, _T("Empty Recycle Bin %s (Y/N)? "), pszRoot);
	while (TRUE) {
		_tprintf(pmt);
		char c = GetEchoChar();
		switch (tolower(c)) {
